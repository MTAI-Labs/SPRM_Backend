"""
Simple script to load akta sections from list_of_akta.txt (no embeddings)
"""
import sys
...
def parse_akta_file(file_path: str) -> list:
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if "AMLFPUAA" in section_code or "AMLATFPUUA" in section_code:
            ...
            elif any(x in section_title.lower() for x in ['suapan', 'rasuah', 'barang berharga']):
                ...
                category = "Penipuan"
            elif any(x in section_title.lower() for x in ['pals', 'dokumen palsu', 'pemalsuan', 'cop', 'mohor']):
                category = "Pemalsuan"
            elif any(x in section_title.lower() for x in ['keterangan', 'sijil', 'bohong']):
...
def load_sections_to_db():
    ...
    print("=" * 60)
    print("Loading Akta Sections (Simple Approach - No Embeddings)")
    print("=" * 60)
    print()

    # Initialize service
    service = AktaSimpleService()
    ...
    print()
    print(f"SUCCESS: Loaded {success_count}/{len(sections)} sections")
    print()
    ...
    for cat in summary:
        print(f"  {cat['category']:<25} : {cat['section_count']:>3} sections")

    print("-" * 60)
    total = service.count_sections()
    print(f"  {'TOTAL':<25} : {total:>3} sections")
    print()
